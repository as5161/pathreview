...
class TestFaithfulnessChecker:
    ...
    def test_mixed_none_and_real_text_chunks(self, checker):
        """A None chunk alongside a real chunk should not blank out the
        real chunk's contribution to the context."""
        feedback = "Skilled with Python programming."
        context_chunks = [
            {"text": "Extensive Python programming experience."},
            {"text": None},
        ]
    ...
    def test_all_chunks_have_none_text(self, checker):
        """If every chunk has text: None, the check should degrade to a
        low/neutral score instead of raising."""
        feedback = "Has Python skills"
        context_chunks = [
            {"text": None},
            {"text": None},
